...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # def mobile_torque(arm1, arm2):
    #     torque1 = length(arm1) * total_weight(end(arm1))
    #     torque2 = length(arm2) * total_weight(end(arm2))
    #     return True if torque1 == torque2 else False
    
    # def balanced_object(obj):
    #     if is_planet(obj):
    #         return True
    #     if is_mobile(obj):
    #         if is_planet(end(left(obj))) and is_planet(end(right(obj))):
    #             return mobile_torque(left(obj), right(obj))
    #         else:
    #             return balanced_object(end(left(obj))) and balanced_object(end(right(obj))) \
    #                  and mobile_torque(left(obj), right(obj))

    # return balanced_object(m)
    if is_planet(m):
        return True
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    # can modify the new_t
    def helper(new_t):
        if is_leaf(new_t):
            if label(new_t) == 'thor':
                new_t[0] = thors_replacement
        else:
