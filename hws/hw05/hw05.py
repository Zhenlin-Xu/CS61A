class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.balance < self.price or self.balance == 0 :
                ...
                self.balance -= self.price
                return f"Here is your {self.name} and ${self.balance} change."
...
def store_digits(n):
    ...
    while n > 9:
        left = n // 10
        right = n % 10
        link = Link(first=right,rest=link)
        n = left
    return Link(n, link)
...
def is_bst(t):
    ...
    #         print(1)
    #         return all([is_bst(t.branches[0])])
    #     else:
    #         print(3)
    #         return False
